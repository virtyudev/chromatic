...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        //const float pad = ImMax(0.0f, (float)(int)(check_sz / 6.5f));
        const float pad = ImMax(0.0f, (float)(int)(check_sz / 8.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 2), grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
